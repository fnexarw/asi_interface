...
class ASiInterface():
    ...
    def __init__(self):
        f1 = os.path.exists("C://Program Files (x86)/AS-Interface Control Tools/BW/en/asidrv32.dll")
        f2 = os.path.exists("C://Program Files/AS-Interface Control Tools/BW/en/asidrv32.dll")
        f3 = os.path.exists("C://ASInterface/asidrv32.dll")
 
        try:
            if f1:                
                self.handler = windll.LoadLibrary(f1)
            elif f2:
                self.handler = windll.LoadLibrary(f2)
            elif f3:
                self.handler = windll.LoadLibrary(f3)
        except Exception:
            self.init_status = -1
